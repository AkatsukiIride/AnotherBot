...
> 版本: 3.1 | 日期: 2026-05-21
> v3.1 更新: QQ音乐指令、流式聊天输出、消息撤回联动、B站/Pixiv数据展示指令、记忆系统
...
| 指令 | 用法 | 说明 |
|------|------|------|
| `/reset` | 直接发送 | 清空当前对话上下文，Bot 忘记前面聊的内容 |
| `/new` | 直接发送 | 结束当前对话，下次 @Bot 开启新话题 |
| `/status` | 直接发送 | 查看当前会话轮数和最近活跃时间 |
| `/help` | 直接发送 | 列出当前平台所有可用指令 |

#### 搜索 & 识图

| 指令 | 用法 | 说明 |
|------|------|------|
| `/search 关键词` | `/search 今天天气` | 联网搜索，Bot 用人设语气回答，含时效判断 |
| `/vision` | 发图片 + `/vision` | 识图后用人设语气点评图片 |

#### B站 (仅 QQ)

| 指令 | 用法 | 说明 |
|------|------|------|
| `/bilibili-hot` | 直接发送 | B站实时热搜榜，角色开场白 + 完整10条热搜 |
| `/bilibili-trending` | 直接发送 | B站热门视频榜，含标题、UP主、播放量、链接 |
| `/bilibili-recommend` | 直接发送 | 随机推荐一个B站视频，含完整信息和链接 |

#### Pixiv (仅 QQ)

| 指令 | 用法 | 说明 |
|------|------|------|
| `/pixiv-recommend` | 直接发送 | Pixiv 日榜随机推荐一张插画 |
| `/pixiv 关键词` | `/pixiv 伊蕾娜` | 搜索 Pixiv 插画，返回最匹配的一张 |

#### 音乐 (仅 QQ，QQ音乐)

| 指令 | 用法 | 说明 |
|------|------|------|
| `/music-hot` | 直接发送 | QQ音乐热歌榜 top 10，含歌名、歌手、链接 |
| `/music-new` | 直接发送 | 新歌榜随机推荐一首 + 角色简评 |
| `/music-search 关键词` | `/music-search 周杰伦` | 搜索 QQ音乐，返回前 5 条结果 |

#### 记忆系统

| 指令 | 用法 | 说明 |
|------|------|------|
| `/remember 键 值` | `/remember 喜欢 面包` | 让 Bot 记住一条偏好/事实 |
| `/list-memory` | 直接发送 | 列出所有已记住的内容（含编号） |
| `/forget 编号/键` | `/forget 1` 或 `/forget 喜欢` | 删除指定记忆 |
| `/learn` | 直接发送 | 收藏上一轮对话为人设示例 |
| `/list-learn` | 直接发送 | 查看已学习的对话示例 |

### 8.4 消息撤回

当你撤回发给 Bot 的消息后，Bot 会自动跟着撤回它的回复。仅在 QQ 平台生效。
...
```
检查:
  1. 群聊需要 @Bot 才回复，私聊直接回复
  2. 是否配置了 AI 模型并设为默认？
  3. 查看日志页，看是否有 API 调用错误
  4. 人设是否已创建并设为活跃？
  5. 指令以 / 开头才触发，普通文字不会误触发指令
```

### Q9: Bot 回复延迟高？

```
Bot 使用流式逐句发送——首句通常 2-5 秒到达，不是等全部生成完再发。
指令（如 /bilibili-trending）涉及外部 API 调用 + LLM 生成，延迟会更高一些，正常现象。
可以检查 AI 模型 API 的响应时间，或换更快的模型（如 qwen-turbo）。
```

### Q10: Bot 回复被截断？

```
群聊消息约 4500 字限制，超长回复会自动分段。
指令回复的数据部分由程序生成（不受 LLM token 限制），
点评部分由 LLM 生成（受 max_tokens 限制）。
如经常截断，可在 Web 管理台调大模型的 max_tokens。
```
